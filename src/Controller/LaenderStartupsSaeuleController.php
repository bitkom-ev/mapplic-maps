...
/**
 * @file
 * Contains \Drupal\mapplic_maps\Controller\LaenderStartupsSaeuleController.
 */
...
/**
 * Class LaenderStartupsSaeuleController
 *
 * @package Drupal\mapplic_maps\Controller
 */
class LaenderStartupsSaeuleController extends ControllerBase {

  public function _mapplic_maps_laenderstartupssaeule_json() {
    $settings = [
      'mapwidth' => "600",
      'mapheight' => "800",
      'categories' => ['Länder Startups Säule'], // /taxonomy/term/825
      'levels' => [],
    ];
    ...
    $query->condition('type', 'mapplic_landmark')
      ->condition('status', 1)
      ->condition('field_mapplic_map_karte', 774) // 774	774	landmark	de	LaenderStartupsSaeule	<p>Länder Startups Säule</p>
      //->condition('field_mapplic_map_karte.entity:taxonomy_term.name', 'LaenderStartupsSaeule', '=') // Länder Startups / Bundeslaender / Deutschland / Europa / Welt
      ->sort('title', 'ASC');
    ...
    if (empty($nodes)) {
      \Drupal::logger('mapplic_maps')
        ->error("Nodes mapplic_landmark and Taxonomy landmark with: LaenderStartupsSaeule are still empty: " . $nodes);
      return new JsonResponse($settings);
    }
