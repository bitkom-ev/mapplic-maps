...
class StartuplandController extends ControllerBase {
  ...
  public function _mapplic_maps_startupland_json() {
    ...
    $query->condition('type', 'mapplic_landmark')
      ->condition('status', 1)
      ->condition('field_mapplic_map_karte', 775) // 775	775	landmark	de	Startupland	<p>Startup-Land</p>
    ...
    if (empty($nodes)) {
      \Drupal::logger('mapplic_maps')
        ->error("Nodes mapplic_landmark and Taxonomy landmark with: Startupland are still empty: " . $nodes);
      return new JsonResponse($settings);
    }
