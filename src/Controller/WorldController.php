...
/**
 * @file
 * Contains \Drupal\mapplic_maps\Controller\WorldController.
 */
...
class WorldController extends ControllerBase {
  ...
  public function _mapplic_maps_world_json() {
    $settings = [
      'mapwidth' => "1200", // 1200 760
      'mapheight' => "760",
      'categories' => ['Welt'],
      'levels' => [],
    ];
    ...
    $query->condition('type', 'mapplic_landmark')
      ->condition('status', 1)
      ->condition('field_mapplic_map_karte', 590) // 590	590	landmark	de	Welt	<p>Weltkarte</p>
    ...
    if (empty($nodes)) {
      \Drupal::logger('mapplic_maps')
        ->error("Nodes mapplic_landmark and Taxonomy landmark with: Welt are still empty: " . $nodes);
